...
class Column(tuple):
    ...
    the CSV file. It should provide for both iteration and random access.
    ...
    small set of possible values ("M/F", "Y/N", etc.). Until 
    """
    
    def map(self, f):
        """Return a new Column by transforming each element with the function
        passed in."""
        return Column(map(f, self))
...
class Document(object):
    ...
    FIXME: We're not storing Unicode for the names. Is this a problem?
    """

    def __init__(self, name_col_pairs):
        """Return a new Document given either an iterable of (String, Column)
        tuples, where the strings are column names; or an OrderedDict. Column 
        names must be unique. Column lengths must be the same.
        """
        # Force expansion in case it's a generator
        name_col_pairs = list(name_col_pairs)
        ...
        # We'll use this as an iterator
        self.Row = namedtuple('Row', self.names, rename=True)
    ...
    def map(self, f):
        return Document((name, col.map(f)) for name, col in self)
